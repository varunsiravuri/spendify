...
A single `$10,000.00` prepaid lot is consumed FIFO. Usage receives credit chronologically by UTC occurrence time, then by event ID, so results do not depend on query ordering. With no invoice received at month-end, the remaining `$2,000.00` becomes a missing-invoice accrual and reverses on September 1.
...
- `supabase/migrations/202609060001_initial_accounting.sql` is the Postgres schema. It enforces non-negative integer micros, idempotency with `unique (provider_id, source_event_id)`, deferred journal balancing with at least two lines, normalized lineage, append-only audit events, and deny-by-default Row Level Security.
...
`supabase db reset` applies the migration and `supabase/seed.sql`. Row Level Security has no public policies in this unauthenticated slice, so application access is deny-by-default. Do not expose `SUPABASE_SERVICE_ROLE_KEY` to client components.
